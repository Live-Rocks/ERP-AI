...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (body?.action === "assign" && typeof body.technicianUserId === "string") {
    ...
    const workOrder = await getFactoryStore().assignWorkOrder(id, technician.id, admin.id);
  ...
  if (body?.action === "resolve" && typeof body.resolution === "string") {
    ...
    const workOrder = await getFactoryStore().resolveWorkOrder(id, technician.id, body.resolution);
