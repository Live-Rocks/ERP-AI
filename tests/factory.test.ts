...
test("模擬 provider 固定回傳五條產線，並以五秒快照更新", () => {
  const { provider } = createFactoryStore();
  const first = provider.refresh(new Date(0));
  const second = provider.refresh(new Date(5000));
...
test("同一未結案模擬異常只建立一張待指派工單", () => {
  const { provider } = createFactoryStore();
  const first = provider.refresh(new Date(0));
  const repeated = provider.refresh(new Date(5000));
...
test("管理員指派後，只有被指派技術員可記錄處置並結案，且保留歷程", async () => {
  ...
  setAuthRepositoryForTests(new MemoryAuthRepository());
  const { provider } = createFactoryStore();
  const created = provider.refresh(new Date(0));
  const workOrderId = created.pendingWorkOrders[0]?.id;
  assert.ok(workOrderId);
  const factory = await import("../src/server/factory-store");
  const globalWorkOrderId = factory.getFactoryProvider().refresh(new Date(0)).pendingWorkOrders[0]?.id;
  ...
  assert.equal(factory.getFactoryStore().historyFor(globalWorkOrderId).length, 3);
});
